        ...
        <div class="bg-gradient-to-br from-teal/10 to-teal/20 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 border border-teal/20">
            <div class="flex items-center">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium text-teal">{{ number_format($activeMembers) }}</span>
                </div>
            </div>
                    </div>
        ...
        <div class="bg-gradient-to-br from-metal-gold/10 to-metal-gold/20 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 border border-metal-gold/20">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($lastServiceAttendance) }}</p>
                    </div>
                </div>
                <div class="mt-4">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Average</span>
                    <span class="font-medium text-metal-gold">{{ number_format($averageAttendance) }}</span>
                </div>
            </div>
                    </div>
        ...
        <div class="bg-gradient-to-br from-crimson/10 to-crimson/20 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 border border-crimson/20">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">GH₵{{ number_format($totalDonationsThisMonth, 2) }}</p>
                    </div>
                </div>
                <div class="mt-4">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">YTD</span>
                    <span class="font-medium text-crimson">GH₵{{ number_format($totalDonationsThisYear, 2) }}</span>
                </div>
            </div>
                    </div>
        ...
        <div class="bg-gradient-to-br from-teal/10 to-teal/20 rounded-xl shadow-lg p-6 transform transition-all duration-200 hover:scale-105 border border-teal/20">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $activeCampaigns->count() }}</p>
                    </div>
                </div>
                <div class="mt-4">
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Total Progress</span>
                    <span class="font-medium text-teal">{{ number_format($campaignProgress, 1) }}%</span>
                </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-lg p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('finance.campaigns.index') }}" class="text-sm text-teal-600 hover:text-teal-800 font-medium">View all</a>
            </div>
            <div class="space-y-6">
                @foreach($activeCampaigns as $campaign)
                    <div class="bg-gray-50 rounded-lg p-4 hover:bg-gray-100 transition-colors duration-200">
                        <div class="flex justify-between mb-2">
                            ...
                            <span class="text-sm font-medium text-teal-600">{{ number_format($campaign->progress_percentage, 1) }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                            <div class="bg-gradient-to-r from-teal-500 to-emerald-500 h-2.5 rounded-full transition-all duration-500" style="width: {{ $campaign->progress_percentage }}%"></div>
                        </div>
                        <div class="flex justify-between mt-2 text-xs text-gray-600">
                            <span>Raised: GH₵{{ number_format($campaign->total_fulfilled + ($campaign->donations->sum('amount') ?? 0), 2) }}</span>
                            <span>Goal: GH₵{{ number_format($campaign->target_amount, 2) }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('members.index') }}" class="text-sm text-teal-600 hover:text-teal-800 font-medium">View all</a>
            </div>
            <div class="space-y-4">
                    ...
                    <div class="flex items-center space-x-4 p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-all duration-200">
                        <div class="flex-shrink-0">
                            @if($member->profile_photo)
                                <img class="h-12 w-12 rounded-full ring-2 ring-teal-500 ring-offset-2" src="{{ asset('storage/' . $member->profile_photo) }}" alt="{{ $member->full_name }}">
                            @else
                                <div class="h-12 w-12 rounded-full bg-gradient-to-br from-teal-500 to-emerald-500 flex items-center justify-center ring-2 ring-teal-500 ring-offset-2">
                                    <span class="text-white text-lg font-medium">{{ substr($member->first_name, 0, 1) }}</span>
                                </div>
                            @endif
                    </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-600">
                                {{ $member->next_birthday->format('M d') }}
                                <span class="text-gray-400 mx-1">&bull;</span>
                                <span class="text-teal-600 font-medium">{{ $member->days_until }}</span> {{ Str::plural('day', $member->days_until) }} away
                            </p>
                    </div>
                    </div>
                @empty
                ...
                @foreach($recentActivities as $activity)
                    <div class="flex items-start p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-all duration-200">
                            ...
                            <span class="inline-flex items-center justify-center h-10 w-10 rounded-lg 
                                {{ $activity->type === 'donation' ? 'bg-rose-100 text-rose-600' : 
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('prayer-requests.index') }}" class="text-sm text-teal-600 hover:text-teal-800 font-medium">View all</a>
            </div>
                <div class="space-y-4">
                @foreach($recentPrayerRequests as $request)
                    <div class="flex items-start p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-all duration-200">
                            <div class="flex-shrink-0">
                            <span class="inline-flex items-center justify-center h-10 w-10 rounded-lg 
                                {{ $request->is_answered ? 'bg-teal-100 text-teal-600' : 'bg-amber-100 text-amber-600' }}">
                        ...
                        <div class="ml-4">
                            <p class="text-sm font-medium text-gray-900">{{ Str::limit($request->title, 50) }}</p>
                            <p class="text-xs text-gray-600 mt-1">
                                By {{ $request->member->full_name }} • 
