...
@section('title', 'Harvest Offerings')
...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('finance.harvest.create') }}" class="btn btn-primary">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div class="card bg-white">
            ...
            <p class="mt-2 text-3xl font-bold" style="color: var(--color-teal)">
                ${{ number_format($statistics->total_harvest ?? 0, 2) }}
            </p>
            <p class="mt-1 text-sm text-gray-500">
                From {{ $statistics->total_events ?? 0 }} events
            </p>
        </div>
        ...
        <div class="card bg-white">
            ...
            <p class="mt-2 text-3xl font-bold" style="color: var(--color-burgundy)">
                ${{ number_format($statistics->average_per_event ?? 0, 2) }}
            </p>
            <p class="mt-1 text-sm text-gray-500">
                {{ $statistics->total_contributors ?? 0 }} contributors
            </p>
        </div>
        ...
        <div class="card bg-white">
            ...
            <p class="mt-1 text-sm text-gray-500">
                Next: {{ $statistics->next_event_date ?? 'None scheduled' }}
            </p>
        </div>
        ...
        <div class="card bg-white">
            ...
            <p class="mt-2 text-3xl font-bold" style="color: var(--color-teal)">
                {{ number_format($statistics->target_achievement ?? 0, 1) }}%
            </p>
            <p class="mt-1 text-sm text-gray-500">
                Of annual target
            </p>
    ...
    <!-- Filters -->
    <div class="card mb-6">
        <form action="{{ route('finance.harvest.index') }}" method="GET" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <select name="harvest_type" id="harvest_type" class="form-input">
                        ...
                        <option value="annual" {{ request('harvest_type') == 'annual' ? 'selected' : '' }}>Annual Harvest</option>
                        <option value="first_fruit" {{ request('harvest_type') == 'first_fruit' ? 'selected' : '' }}>First Fruit</option>
                        <option value="special" {{ request('harvest_type') == 'special' ? 'selected' : '' }}>Special Harvest</option>
                        <option value="thanksgiving" {{ request('harvest_type') == 'thanksgiving' ? 'selected' : '' }}>Thanksgiving</option>
                    </select>
                </div>
    ...
    <div class="card">
                    ...
                    @forelse($harvests ?? [] as $harvest)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $harvest->name }}</div>
                                <div class="text-sm text-gray-500">{{ $harvest->theme }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                {{ $harvest->date->format('M d, Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                    {{ ucfirst($harvest->type) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium" style="color: var(--color-burgundy)">
                                ${{ number_format($harvest->target_amount, 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium" style="color: var(--color-teal)">
                                    ${{ number_format($harvest->collected_amount, 2) }}
                                </div>
                                <div class="text-xs text-gray-500">
                                    {{ number_format(($harvest->collected_amount / $harvest->target_amount) * 100, 1) }}% of target
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <div class="flex space-x-2">
                                    <a href="{{ route('finance.harvest.show', $harvest) }}" class="text-indigo-600 hover:text-indigo-900">
                                        View
                                    </a>
                                    @if($harvest->status !== 'completed')
                                        <a href="{{ route('finance.harvest.edit', $harvest) }}" class="text-yellow-600 hover:text-yellow-900">
                                            Edit
                                        </a>
                                        <form action="{{ route('finance.harvest.delete', $harvest) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
        ...
        @if(isset($harvests) && $harvests->hasPages())
            <div class="mt-4">
                {{ $harvests->links() }}
            </div>
        @endif
    ...
    @if(isset($upcoming_harvests) && count($upcoming_harvests) > 0)
        <div class="mt-6">
            <h3 class="text-lg font-medium mb-4">Upcoming Harvest Events</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach($upcoming_harvests as $upcoming)
                    <div class="card">
                        <div class="flex justify-between items-start">
                            <div>
                                <h4 class="font-medium">{{ $upcoming->name }}</h4>
                                <p class="text-sm text-gray-500">{{ $upcoming->date->format('M d, Y') }}</p>
                            </div>
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                {{ ucfirst($upcoming->type) }}
                            </span>
                        </div>
                        <div class="mt-4">
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-500">Target:</span>
                                <span class="font-medium">${{ number_format($upcoming->target_amount, 2) }}</span>
                            </div>
                            <div class="flex justify-between text-sm mt-1">
                                <span class="text-gray-500">Pledges:</span>
                                <span class="font-medium">${{ number_format($upcoming->pledged_amount, 2) }}</span>
                            </div>
                        </div>
                        <div class="mt-4">
                            <div class="w-full bg-gray-200 rounded-full h-2">
                                <div class="h-2 rounded-full" style="width: {{ ($upcoming->pledged_amount / $upcoming->target_amount) * 100 }}%; background-color: var(--color-teal)"></div>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">
                                {{ number_format(($upcoming->pledged_amount / $upcoming->target_amount) * 100, 1) }}% pledged
                            </p>
                        </div>
                    </div>
                @endforeach
