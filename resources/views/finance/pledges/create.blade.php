...
@section('title', 'Create New Pledge')
...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('finance.pledges.index') }}" class="text-gray-700 hover:text-blue-600">
                        Pledges
                    </a>
                </div>
            </li>
    ...
    <div class="bg-white shadow-sm rounded-lg">
        <div class="p-6">
            ...
            <form action="{{ route('finance.pledges.store') }}" method="POST" class="space-y-8">
                @csrf
                                ...
                                <select name="campaign_id" id="campaign_id" 
                                    ...
                                    @foreach($campaigns as $campaign)
                                        <option value="{{ $campaign->id }}" 
                                            ...
                                            {{ $campaign->name }} 
                                            (Target: GH₵{{ number_format($campaign->target_amount, 2) }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('campaign_id')
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select name="payment_schedule" id="payment_schedule" 
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                            required>
                                        <option value="weekly" {{ old('payment_schedule') == 'weekly' ? 'selected' : '' }}>Weekly</option>
                                        <option value="biweekly" {{ old('payment_schedule') == 'biweekly' ? 'selected' : '' }}>Bi-weekly</option>
                                        ...
                                        <option value="custom" {{ old('payment_schedule') == 'custom' ? 'selected' : '' }}>Custom Schedule</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="pledge_date" id="pledge_date" 
                                           value="{{ old('pledge_date', date('Y-m-d')) }}"
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                           required>
                                    @error('pledge_date')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input type="checkbox" name="send_reminders" id="send_reminders" 
                                               ...
                                               {{ old('send_reminders', true) ? 'checked' : '' }}>
                                    </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="notes" id="notes" rows="3" 
                                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                              placeholder="Any additional information about this pledge...">{{ old('notes') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
