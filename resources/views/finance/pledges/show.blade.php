        ...
        <div class="border-t border-gray-200">
            <dl>
                ...
                <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        {{ ucfirst($pledge->payment_schedule) }}
                    </dd>
                </div>
                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt class="text-sm font-medium text-gray-500">Start Date</dt>
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        {{ $pledge->start_date->format('F j, Y') }}
                    </dd>
                </div>
                <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt class="text-sm font-medium text-gray-500">End Date</dt>
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        {{ $pledge->end_date ? $pledge->end_date->format('F j, Y') : 'N/A' }}
                    </dd>
                </div>
                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        @foreach($pledge->payments as $payment)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <form action="{{ route('finance.pledges.payments.destroy', ['pledge' => $pledge, 'payment' => $payment]) }}" 
    ...
    <div id="recordPaymentModal" class="hidden fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form action="{{ route('finance.pledges.payments.store', $pledge) }}" method="POST">
                    @csrf
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="amount" id="amount" step="0.01" required
                                        ...
                                        value="{{ number_format($pledge->amount - $pledge->amount_fulfilled, 2, '.', '') }}">
                                </div>
                            </div>
                            <div>
