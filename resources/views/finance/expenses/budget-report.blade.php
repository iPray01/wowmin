...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <form action="{{ route('finance.expenses.budget-report') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Report Type -->
            <div>
    ...
    <div class="space-y-6">
        @foreach($reportData as $period)
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-xl font-semibold text-gray-800 mb-4">{{ $period['month'] }}</h2>

                <!-- Total Spending Overview -->
                <div class="mb-6">
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-sm font-medium text-gray-600">Total Spending</span>
                        <span class="text-lg font-semibold text-gray-900">GH₵{{ number_format($period['total_spent'], 2) }}</span>
                    </div>
                </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    ...
                                    <span class="text-sm font-medium text-gray-900">
                                        GH₵{{ number_format($period['department_expenses'][$department->name] ?? 0, 2) }}
                                    </span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-teal-600 h-2 rounded-full" 
                                         style="width: {{ $period['total_spent'] > 0 ? (($period['department_expenses'][$department->name] ?? 0) / $period['total_spent']) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach($categories as $category)
                            <div>
                                <div class="flex justify-between items-center mb-1">
                                    ...
                                    <span class="text-sm font-medium text-gray-900">
                                        GH₵{{ number_format($period['category_expenses'][$category->name] ?? 0, 2) }}
                                    </span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2">
                                    <div class="bg-indigo-600 h-2 rounded-full" 
                                         style="width: {{ $period['total_spent'] > 0 ? (($period['category_expenses'][$category->name] ?? 0) / $period['total_spent']) * 100 : 0 }}%">
                                    </div>
                                </div>
                            </div>
                        @endforeach
    ...
    <div class="mt-6 flex justify-end space-x-4">
        <a href="{{ route('finance.expenses.budget-report', array_merge(request()->query(), ['export' => 'pdf'])) }}" 
           class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700">
            <i class="fas fa-file-pdf mr-2"></i>
            Export as PDF
        </a>
        <a href="{{ route('finance.expenses.budget-report', array_merge(request()->query(), ['export' => 'excel'])) }}" 
